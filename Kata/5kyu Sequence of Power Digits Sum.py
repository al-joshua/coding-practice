...
We can observe that it took 3 steps to reach a cyclical pattern [153](h = 3). The length of this cyclical
pattern is 1, patt_len. The last term of our k operations is 153, last_term
...
[13139, 6725, 4338, 4514, 1138, 4179, 9219] with a length of 7, (patt_len = 7),
and it took 12 steps (h = 12) to reach the cyclical pattern. The last term after doing 30 operations is 1138
...
def sum_pow_dig_seq(start, n, k):
    ...
    for i in range(k):
        arr.append(start)
        start = sum(int(s) ** n for s in str(start))

        if start in arr and not cyc_patt_arr:
            h = arr.index(start)
            cyc_patt_arr = arr[arr.index(start):]
